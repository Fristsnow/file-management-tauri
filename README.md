...
git remote add origin https://github.com/Fristsnow/file-management-tauri.git

第一周： 客户端：登录 文件展示, 上传, 下载

第二周:  web端：用户管理模块(权限管理, 角色管理, 用户管理),

第三周:  web端：部门管理模块, 文档管理模块, 

第四周:  web端: 日志管理模块, 统计信息, 客户端: mac打包
